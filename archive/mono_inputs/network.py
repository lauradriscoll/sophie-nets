...
def popvec(y):
    ...
    return np.mod(np.pi - loc, 2*np.pi)
...
def tf_popvec(y):
    ...
    return tf.mod(np.pi - loc, 2*np.pi)
...
class LeakyGRUCell(RNNCell):
  ...
  def __init__(self,
    ...
    matrix0 = np.concatenate((w_in0, w_rec0), axis=0)
    matrix1 = np.concatenate((matrix0, matrix0), axis=1)

    self.w_rnn0 = matrix0
    self.w_rnn1 = matrix1
    self._initializer_candidate = tf.constant_initializer(matrix0, dtype=tf.float32)
    self._initializer_gate = tf.constant_initializer(matrix1, dtype=tf.float32)
  ...
  def build(self, inputs_shape):
      ...
      self._gate_kernel = self.add_variable(
          ...
          initializer=self._initializer_gate)
      ...
      self._candidate_kernel = self.add_variable(
          "candidate/%s" % 'kernel',
          shape=[input_depth + self._num_units, self._num_units],
          initializer=self._initializer_candidate)
...
class Model(object):
    ...
    def _build_fused(self, hp):
        ...
        if 'w_rec_coeff' in hp:
            w_rec_coeff = hp['w_rec_coeff']
        else:
        ...
        self.x = self.datasetTensors[0]
        self.y = self.datasetTensors[1]
        self.c_mask = self.datasetTensors[2]
        ...
        if hp['rnn_type'] == 'LeakyRNN':
            ...
            self.cell = LeakyGRUCell(n_rnn, n_in_rnn,
                                hp['alpha'],
                                sigma_rec=hp['sigma_rec'],
                                activation=hp['activation'],
                                w_rec_init=hp['w_rec_init'],
            ...
            if hp['w_rec_init'] == 'diag':
                initializer = tf.keras.initializers.Identity(gain=w_rec_coeff)
            elif hp['w_rec_init']== 'randgauss':
                initializer = tf.keras.initializers.RandomNormal(
                    mean=w_rec_coeff,
                    seed=None,
                    stddev=1/np.sqrt(n_rnn))
            self.cell = tf.contrib.rnn.GRUCell(n_rnn, activation=f_act, kernel_initializer = initializer)
        else:
