...
arm_dict = np.load('/Users/lauradriscoll/Documents/data/rnn/multitask/armNet/arm_dict_800.npy').item() #interpolated arm dict from frank
    ...
    def add(self, loc_type, locs=None, ons=None, offs=None, strengths=1, mods=None):
        ...
        for i in range(self.batch_size):
            if loc_type == 'fix_in':
                ...
                self.y[ons[i]: offs[i], i, 1:] = arm_dict['outputs'][0,inds,:]
            elif loc_type == 'out':
                total_inds = np.shape(self.y[ons[i]:offs[i],i,1:])[0]
                max_inds = 150
                min_inds = 50
...
def _contextdm(config, mode, attend_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        tdim = int(2000/dt)
        n_stim_loc, n_stim_mod1_strength, n_stim_mod2_strength = batch_shape = 50, 5, 5
        batch_size = np.prod(batch_shape)
...
def _dm(config, mode, stim_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        tdim = int(2500/dt)
        n_stim_loc, n_stim1_strength = batch_shape = 200, 5
        batch_size = np.prod(batch_shape)
...
def _contextdelaydm(config, mode, attend_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim2_ons  = stim1_offs + int(delay_fac*rng.choice([200, 400, 800, 1600])/dt)
        ...
        n_stim_loc, n_stim_mod1_strength, n_stim_mod2_strength = batch_shape = 200, 5, 5
        batch_size = np.prod(batch_shape)
        ind_stim_loc, ind_stim_mod1_strength, ind_stim_mod2_strength = np.unravel_index(range(batch_size),batch_shape)
...
def datasetGeneratorFromTaskDef(hp, mode):
   ...
   while True:
       ...
       datasetTensors = generate_datasetTensors(rule_train_now, hp, mode, batch_size = hp['batch_size_train'], delay_fac=hp['delay_fac'])
       dtEnd = datetime.now()
