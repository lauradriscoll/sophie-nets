...
def gen_trials_from_model_dir(model_dir,rule,mode='test',noise_on = True,batch_size = 500,alpha_mod1 = 0):
...
#         params = [sess.run(var) for var in var_list]
        ...
        hparams = model.hp
        # create a trial
        trial = generate_trials(rule, hparams, mode=mode, noise_on=noise_on, batch_size =batch_size, delay_fac =1,alpha_mod1 = alpha_mod1)
    return trial  
...
def make_Jac_u_dot_delu(model_dir_all,ckpt_n_dir,rule,task_set,time_set,trial_set,alpha_mod1 = 0):
    ...
    with tf.Session() as sess:
        ...
        # get hparams
        hparams = model.hp
        trial = generate_trials(rule, hparams, mode='test', noise_on=False, delay_fac =1,alpha_mod1 = alpha_mod1)
...
def make_Jac_x(model_dir_all,ckpt_n_dir,rule,task_set,time_set,trial_set,alpha_mod1 = 0):
    n_tasks = len(task_set)
...
def make_h_and_Jac(model_dir_all,ckpt_n_dir,rule,task_set,time_set,trial_set,alpha_mod1 = 0):
    ...
    with tf.Session() as sess:
        ...
        # get hparams
        hparams = model.hp
        trial = generate_trials('delaygo', hparams, mode='test', noise_on=False, delay_fac =1,alpha_mod1 = alpha_mod1)
...
def make_h_combined(model_dir_all,ckpt_n_dir,tasks,trial_set,epoch,alpha_mod1 = 0):
    ...
    with tf.Session() as sess:
        ...
        # get hparams
        hparams = model.hp
        trial = generate_trials(rule, hparams, mode='random', batch_size = 100,noise_on=True, delay_fac =1,alpha_mod1 = alpha_mod1)
...
def generate_Beta_epoch(h_tf,trial,ind = -1,mod = 'either'):
    ...
    for epoch in trial.epochs.keys():
        ...
        y1 = np.expand_dims(np.cos(angle_var),axis = 1)
        y2 = np.expand_dims(np.sin(angle_var),axis = 1)
...
def make_h_trial_rule(model_dir_all,mode = 'random',noise_on = False,alpha_mod1 = 0):
    ...
    with tf.Session() as sess:
        ...
        for rule in hp['rule_trains']:
            trial[rule] = generate_trials(rule, hp, mode,
                            batch_size=400, delay_fac=hp['delay_fac'],noise_on = noise_on,alpha_mod1 = alpha_mod1)
...
def plot_epoch_dynamics(fig,m,fp_file,epoch,h,trial,rule,D_use,
                        y_set = range(0,360,36),h_epoch = [],plot_eigenspect = True,lim=[], ax_type = 'tdr',
...
#         nc = 1
    ...
    for t in [0,]:#range(0,np.shape(h[rule])[1],int(np.shape(h[rule])[1]/10)):
    ...
    for ind_stim_loc in y_set:
        fps, J_xstar = make_fp_struct(m,fp_file,rule,epoch,ind_stim_loc)
        eig_decomps = comp_eig_decomp(J_xstar)
        fps_tdr = np.dot(fps,D_use)
        fp_c = cmap(ind_stim_loc/(360))
        plt.plot(fps_tdr[:,0],fps_tdr[:,1],'o',c = 'k',alpha = .5,markersize=6)
        plot_FP(fps, D_use.T, eig_decomps, c = 'k')

        if plot_eigenspect:
            ax2 = fig.add_axes([.55, .45, .2, .2])
...
def plot_FP_jitter(m,D_use,rule_master,t_num,fp_epoch,sorted_fps,fp_inds,rule_set,
                   ...
                   lw = 3,al = .6,linestyle = '-',n_jit = 0,alpha_mod1 = 0):
    ...
    with tf.Session() as sess:
        ...
        for rule in rule_set:
            trial = generate_trials(rule, hparams, mode='test',noise_on=False,alpha_mod1 = alpha_mod1)
            feed_dict = tools.gen_feed_dict(model, trial, hparams)
        ...
        trial = generate_trials(rule_master, hparams, mode='test',noise_on=False,alpha_mod1 = alpha_mod1)
        feed_dict = tools.gen_feed_dict(model, trial, hparams)
