...
for rule in task_list:
    ...
    with tf.Session() as sess:
        ...
        # hyperparameters.
        fpf_hps = ({'tol_q': 1e-12,'max_iters': 50000})
        ...
        for t in range(1):#range(1,n_trials): np.arange(0, 40, 8): #0,n_trials,40
            ...
            for step_i in range(n_interp):
                ...
                if np.shape(fp_predictions)[0]==0:
                    fp_predictions = fpf.sample_states(example_predictions['state'][:,[-1,],:],
                                                n_inits=N_INITS,
                ...
                if unique_fps.xstar.shape[0]>0:
                    ...
                    save_dir = os.path.join(model_dir_all,'fixed_pts_stepX_reverse',rule)#,'random_trials'
