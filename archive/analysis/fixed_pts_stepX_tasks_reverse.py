...
import tools
from tools_lnd import plot_N, plot_FP, name_best_ckpt, gen_trials_from_model_dir, align_output_inds
...
with tf.Session() as sess:
    ...
    h_tf_cat1 = h_tf1[int(e_lims[0,0]):int(e_lims[1,0]),:,:]
    h_tf_cat2 = h_tf2[int(e_lims[0,1]):int(e_lims[1,1]),:,:]
    ...
    for t in range(1):#range(1,n_trials): np.arange(0, 40, 8): #0,n_trials,40
        ...
        inputs_2 = trial2.x[int(e_lims[0,1]),t,:]
        del_inputs = inputs_2 - inputs_1

        for step_i in range(n_interp):

            step_inputs = inputs_2[np.newaxis,:]-del_inputs[np.newaxis,:]*(step_i/n_interp)
            inputs = step_inputs
            ...
            if unique_fps.xstar.shape[0]>0:
                ...
                save_dir = os.path.join(model_dir_all,'fixed_pts_stepX_tasks_reverse',epoch,task_list[0]+'_'+task_list[1])#,'random_trials'
