...
l1w = 0
l1h = 0
seed = 'untrained'#str(0)
net_name = 'l2_w'+"{:.1f}".format(-l2w)+'_h'+"{:.1f}".format(-l2h)+'_'+rule_trains_str
data_folder = 'data/rnn/multitask/stepnet/variable_stim'

model_dir_all = os.path.join(p,data_folder,rnn_type,activation,w_init,n_tasks+'_tasks',n_rnn+'_n_rnn',net_name,seed)
h_cat = make_cat_h_rules(model_dir_all).T

task_list = rules_dict['all']
...
with tf.Session() as sess:
    ...
    # hyperparameters.
    fpf_hps = {'tol_q': 1e-7,'tol_unique': 1e-1}
