...
import tools
from tools_lnd import gen_trials_from_model_dir, align_output_inds, same_stim_trial
...
'''Initial states are sampled from states obsserved during realistic behavior
    ...
    for t in range(1):#range(1,n_trials): np.arange(0, 40, 8): #0,n_trials,40
        ...
        for step_i in range(n_interp):
            ...
            if unique_fps.xstar.shape[0]>0:
                ...
                save_dir = os.path.join(model_dir_all,'fixed_pts_stepX_tasks_fwd_stim',epoch,task_list[0]+'_'+task_list[1])#,'random_trials'
