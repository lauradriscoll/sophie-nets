...
eventIdx = eventIdx(task == 1 | task == 4);%eventIdx(task == 2);% eventIdx(task == 3 | task == 2);%eventIdx(task~=2 & task~=5); %
task = conds(eventIdx,1);
target = conds(eventIdx,2);

trialCodes = cat(2,task == 1,target); %target;%cat(2,task == 3,target); %cat(2,task>3,mod(task,3));%
%%
timeWindow = [-100 100];
timeStep = 0.02;
margNames=[];
...
for feat = 1:4
    hold on;
    plot(squeeze(out.featureAverages(:,1,feat,:))'*cond_indep,'-')
    plot(squeeze(out.featureAverages(:,2,feat,:))'*cond_indep,':')
end
...
for feat = 1:4
    hold on;
    plot(squeeze(out.featureAverages(:,1,feat,:))'*task_sep,'-')
    plot(squeeze(out.featureAverages(:,2,feat,:))'*task_sep,':')
end