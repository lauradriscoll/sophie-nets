...
##################################################################
model_n = 1
dir_specific_all = 'stepnet/crystals/softplus/l2w0001'#'crystals/softplus/no_reg'#''crystals/softplus/l2h00001'#'stepnet/crystals/softplus/'#grad_norm_both/'#'lowD/combos'#'stepnet/lowD/tanh'#'lowD/grad_norm_l2001' #' #'lowD/armnet_noreg'#lowD/combos' ##grad_norm_l2h000001' /Documents/data/rnn/multitask/varGo/lowD/most/
        ...
        for t in range(1):#range(1,n_trials): np.arange(0, 40, 8): #0,n_trials,40
            ...
            for step_i in range(n_interp):
                ...
                if unique_fps.xstar.shape[0]>0:
                    ...
                    all_states = np.reshape(example_predictions['state'],(-1,n_rnn))
                    fp_predictions = fpf.sample_states(np.concatenate((all_states[np.newaxis,:,:],unique_fps.xstar[np.newaxis,:,:]),axis=1),
                                                n_inits=N_INITS,
                                                noise_scale=NOISE_SCALE)

                    save_dir = os.path.join(model_dir_all,'fixed_pts_stepX_init_all_states',rule)#,'random_trials'
