...
w_init = 'diag'
ruleset = 'pro_small'
task_list = rules_dict[ruleset]
rule_trains_str = '_'.join(task_list)
n_tasks = str(len(task_list))
n_rnn = str(256)
l2w = -6
l2h = -6
l1w = 0
...
# model_dir_all = os.path.join(p,data_folder,ruleset,rnn_type,activation,w_init,n_tasks+'_tasks',n_rnn+'_n_rnn',net_name,seed)

model_dir_all = os.path.join(p,data_folder,rnn_type,activation,w_init,ruleset,n_tasks+'_tasks',
    n_rnn+'_n_rnn',net_name,'post_train_'+post_train,seed)
...
n_interp = 5 # number of steps between input conditions
...
with tf.Session() as sess:
    ...
    same_inds = inputs_1==inputs_2
    diff_inds = inputs_1!=inputs_2

    zero_vec = np.zeros(np.shape(inputs_1[np.newaxis,:]))
    same_vec = zero_vec
    same_vec[0,same_inds] = inputs_1[np.newaxis,same_inds]
    
    zero_vec = np.zeros(np.shape(inputs_1[np.newaxis,:]))
    diff_vec1 = zero_vec
    diff_vec1[0,diff_inds] = inputs_1[np.newaxis,diff_inds]
    
    zero_vec = np.zeros(np.shape(inputs_1[np.newaxis,:]))
    diff_vec2 = zero_vec
    diff_vec2[0,diff_inds] = inputs_2[np.newaxis,diff_inds]

    for step_1i in range(n_interp):
        for step_2i in range(n_interp):

            step_inputs = same_vec+diff_vec1*(step_1i/n_interp)+diff_vec2*(step_2i/n_interp)
            inputs = step_inputs
            ...
            if unique_fps.xstar.shape[0]>0:
                ...
                np.savez(os.path.join(save_dir,filename+'_step_a'+str(step_1i)+'_step_b'+str(step_2i)+'.npz'),**all_fps)
