...
def get_filename(trial,epoch_set,t):
    ...
    ind_stim_loc  = np.min(trial.stim_locs[t,[0,2]])
    filename = rule+'_'+epoch_set[0]+'_'+epoch_set[1]+'_'+str(int(ind_stim_loc*180/np.pi))
...
for model_n in [0,1]:
    ...
    N_INITS = 10000 # The number of initial states to provide
    ...
    with tf.Session() as sess:
        ...
        for t in [64,]:#range(1):#range(1,n_trials): np.arange(0, 40, 8): #0,n_trials,40
            print(t)
            ...
            for step_i in range(n_interp):
                ...
                if unique_fps.xstar.shape[0]>0:
                    ...
                    filename,_ = get_filename(trial,epoch_set,t)
