...
for seed in seed_set:
    for ruleset in ['pro_small',]:

     #    rule_trains_pre = ['fdgo', 'reactgo', 'delaygo', 'fdanti', 'reactanti', 'delayanti','delaydm1', 'delaydm2', 'contextdelaydm1', 'contextdelaydm2', 'multidelaydm','dmsgo', 'dmsnogo', 'dmcgo', 'dmcnogo']
     #    rule_trains_pre.pop(pop_rule)
     #    n_tasks = str(len(rule_trains_pre))
	    # post_train = rule_trains_all[pop_rule]

     #    s = '_'
     #    rule_trains_str = s.join(rule_trains_pre)
        rule_trains_str = s.join(rules_dict[ruleset])

        net_name = 'lr'+"{:.1f}".format(-lr)+'l2_w'+"{:.1f}".format(-l2w)+'_h'+"{:.1f}".format(-l2h)+'_'+rule_trains_str

        m = os.path.join(p,data_folder,ruleset,rnn_type,activation,w_init,n_tasks+'_tasks',n_rnn+'_n_rnn',net_name,seed)

        for rule in [post_train,]:
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(n_epochs):
                    ...
                    for t in range(0,int(n_trials),int(n_trials/2)):#[int(n_trials/2),]:#:2,n_trials,int(n_trials/4)
