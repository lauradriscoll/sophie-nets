...
#         self.y = np.zeros((tdim, batch_size, self.n_output), dtype=self.float_type)
        ...
        for i in range(self.batch_size):
            if loc_type == 'fix_in':
                ...
                if self.config['loss_type'] == 'lsq':
                    ...
                    y_tmp /= np.sum(y_tmp)
                    self.y[ons[i]: offs[i], i, 1:] += .8*y_tmp # shrink target to prevent saturation of some act fxns 20190522
    ...
    def add_rule(self, rule, on=None, off=None, strength=1., **kwargs):
        """Add rule input."""
        if isinstance(rule, int):
            self.x[on:off, :, self.config['rule_start']+rule] = strength

        elif rule == 'contin_delaydm12':

            s1 = kwargs['alpha_mod1']
            ind_rule = int(round(s1))
            self.x[on:off, :, ind_rule+self.config['rule_start']] = strength
...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim_offs = stim_ons + int(rng.uniform(200,600)/dt) #int(rng.choice([200, 400, 600])/dt) # dec 14 2018
        fix_offs = stim_offs + int(delay_fac*rng.uniform(200,1600)/dt) #int(rng.choice([200, 400, 800, 1600])/dt) # dec 14 2018
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        if (attend_mod == 1) or (attend_mod == 2):
            ...
            stim1_mod12_diff = stim1_strengths * \
                               ...
                               np.random.choice([+1, -1], (batch_size,)) #Min diff changed from .2 to .005 20190805
        ...
        stim_dur = int(rng.uniform(400,1600)/dt) #changed from discrete 20190805
        ...
        tdim = stim_on+stim_dur+delay_dur + int(rng.uniform(300,700)/dt) # 20190510
...
def reactgo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        tdim = stim_ons + int(rng.uniform(300,700)/dt) # 20190510
...
def fdgo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        fix_offs  = stim_ons + int(delay_fac*rng.uniform(500,1500)/dt)
        tdim      = fix_offs + int(rng.uniform(300,700)/dt) # 20190510
...
def _dm(config, mode, stim_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim_dur = int(rng.uniform(400,1600)/dt) #int(rng.choice([400, 800, 1600])/dt) dec 19th, 2018
        fix_offs = (stim_ons+stim_dur).astype(int)
        # each batch consists of sequences of equal length
        tdim = stim_on+stim_dur + int(rng.uniform(300,700)/dt) # 20190510
...
def _delaydm(config, mode, stim_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim2_ons  = stim1_offs + int(delay_fac*rng.uniform(200,1600)/dt) #int(rng.choice([200, 400, 800, 1600])/dt) # dec 17 2018
        stim2_offs = stim2_ons + int(rng.uniform(200,600)/dt) # int(rng.choice([200, 400, 600])/dt) #dec 19th 2018
...
def _contextdelaydm(config, mode, attend_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        if (attend_mod == 1) or (attend_mod == 2):
            ...
            # from (0.005, 0.8, (batch_size,)) to .2 task is harder 20200727
            stim1_mod12_diff = stim1_strengths * \
        ...
        stim2_ons  = stim1_offs + int(delay_fac*rng.uniform(200,1600)/dt)
        stim2_offs = stim2_ons + int(rng.uniform(200,600)/dt)
        fix_offs  = stim2_offs + int(rng.uniform(100,300)/dt)
    ...
    if attend_mod == 1:
        ...
        stim1_strengths = stim1_mod1_strengths + stim1_mod2_strengths
        stim2_strengths = stim2_mod1_strengths + stim2_mod2_strengths
    elif attend_mod == 'alpha_mod1':
        s1 = kwargs['alpha_mod1']/19
        s2 = 1-s1
        stim1_strengths = s1*stim1_mod1_strengths + s2*stim1_mod2_strengths
        stim2_strengths = s1*stim2_mod1_strengths + s2*stim2_mod2_strengths
...
def contin_delaydm12(config, mode, **kwargs):
    return _contextdelaydm(config, mode, 'alpha_mod1', **kwargs)
...
def dms_(config, mode, matchnogo, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        n_stim_loc, n_mod1, n_mod2 = batch_shape = 200, 2, 2
        batch_size = np.prod(batch_shape)
...
rule_mapping = {'testinit': test_init,
                ...
                'contin_delaydm12': contin_delaydm12,
                'dmsgo': dmsgo,
                'dmsnogo': dmsnogo,
                'dmcgo': dmcgo,
                'dmcnogo': dmcnogo,
...
rule_name    = {'reactgo': 'RT Go',
                ...
                'contin_delaydm12': 'Continuous MultSen Dly DM',
                'reactanti': 'RT Anti',
                'delayanti': 'Dly Anti',
                'fdanti': 'Anti',
                'dmsgo': 'DMS',
...
def generate_trials(rule, hp, mode, noise_on=True, **kwargs):
    ...
    for r, s in zip(rule, rule_strength):
        trial.add_rule(r, on=rule_on, off=rule_off, strength=s, alpha_mod1 = kwargs['alpha_mod1'])
...
def datasetGeneratorFromTaskDef(hp, mode):
   ...
   while True:
       ...
       datasetTensors = generate_datasetTensors(rule_train_now, hp, mode, \
        batch_size = hp['batch_size_train'], delay_fac=hp['delay_fac'], alpha_mod1 = hp['rng'].randint(20))
       dtEnd = datetime.now()
