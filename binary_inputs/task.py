...
class Trial(object):
    ...
    def add(self, loc_type, locs=None, ons=None, offs=None, strengths=1, mods=1):
        ...
        #only doing 1 mod for now
        mods = [1]*self.batch_size
    ...
    def smooth_input(self,window_range = 20):
        """Change inputs to graded"""
        from scipy import ndimage as ndi
        window = np.random.randint(1, window_range)
...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        tdim = int(2500/dt)
        n_stim_loc, n_stim_mod = batch_shape = config['n_eachring'], 1
...
def fdgo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        tdim = int(2000/dt)
        n_stim_loc, n_stim_mod = batch_shape = 2, 1
        batch_size = np.prod(batch_shape)
        ind_stim_loc, ind_stim_mod = np.unravel_index(range(batch_size),batch_shape)
