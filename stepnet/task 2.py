...
class Trial(object):
    ...
    def add_x_loc(self, x_loc):
        ...
        # """Stepnet"""
        # x = np.array((np.sin(x_loc), np.cos(x_loc)))
...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        # """Binary""" pick stim locs centered on prefs
...
+        stim2_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        ...
        # """Binary""" 
        n_stim_loc, n_stim1_strength = batch_shape = config['n_eachring'], 4
...
def dms_(config, mode, matchnogo, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        stim1_mod  = 1#rng.choice([1,2])
        stim2_mod  = 1#rng.choice([1,2])
...
+        stim2_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        ...
        # """Binary""" 
        n_stim_loc, n_mod1, n_mod2 = batch_shape = config['n_eachring'], 2, 2
