...
for rnn_type in ['LeakyRNN',]: #'LeakyRNN',
    for activation in ['tanh',]: #,'retanh'
        for init in ['randgauss',]:# 'randgauss'
            for n_rnn in [128,]:
                for seed in [0,]: #['untrained',]:#['untrained',]:
                    ...
                    for rule_trains_label in rule_set_keys:
                        ...
                        m = os.path.join(p,'data','rnn','multitask',net,'final1',rule_trains_label,rnn_type,activation,
                            init,str(len(rule_trains))+'_tasks',str(n_rnn)+'_n_rnn',net_name+net_name2,str(seed))

                        # m = os.path.join(p,'data','rnn','multitask',net,rule_trains_label,rnn_type,activation,
                        #     init,str(len(rule_trains))+'_tasks',str(n_rnn)+'_n_rnn',net_name+net_name2,str(seed))

                        # net_name = 'lr'+"{:.1f}".format(-lr)+'l2_w'+"{:.1f}".format(-l2w)+'_h'+"{:.1f}".format(-l2h)+'_'+rule_trains_str
                        # m = os.path.join(p,'data','rnn','multitask',net,'final','all',rnn_type,activation,init,str(len(rule_trains))+'_tasks',str(n_rnn)+'_n_rnn',net_name,str(seed))
