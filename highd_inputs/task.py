...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        delay_fac = 1#kwargs['delay_fac']

        # A list of locations of stimuluss and on/off time
        # stim_locs = rng.rand(batch_size)*2*np.pi
        # pick stim locs centered on prefs
        stim_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        # stim_ons  = int(500/dt)
        ...
        tdim = int(2500/dt)
        n_stim_loc, n_stim_mod = batch_shape = config['n_eachring']*200, 2
...
def _contextdm(config, mode, attend_mod, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        # pick stim locs centered on prefs
        stim1_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        stim2_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        ...
        tdim = int(2000/dt)
        n_stim_loc, n_stim_mod1_strength, n_stim_mod2_strength = batch_shape = config['n_eachring']*200, 4, 4
        batch_size = np.prod(batch_shape)
...
def dms_(config, mode, matchnogo, **kwargs):
    ...
    if mode == 'random': # Randomly generate parameters
        ...
        # pick stim locs centered on prefs
        stim1_locs = rng.choice(range(config['n_eachring']),batch_size)*(2*np.pi/config['n_eachring'])
        if config['n_eachring']==2:
            stim_dist  = np.pi*np.ones(batch_size) # non match automatically goes to the other side 
        else:
            stim_dist  = rng.choice(range(1,config['n_eachring']-1),batch_size)*(2*np.pi/config['n_eachring'])
            stim2_locs = (stim1_locs+stim_dist*(1-matchs))%(2*np.pi)
...
def datasetGeneratorFromTaskDef(hp, mode):
   ...
   while True:
       ...
       datasetTensors = generate_datasetTensors(rule_train_now, hp, mode, batch_size = hp['batch_size_train'], delay_fac=1)#hp['delay_fac'])
       dtEnd = datetime.now()
